...
<head>
    ...
    <link href="https://fonts.googleapis.com/css2?family=Aref+Ruqaa:wght@400;700&family=Tajawal:wght@300;400;500&family=Great+Vibes&display=swap" rel="stylesheet">
    ...
    <style>
        ...
        .font-ink { font-family: 'Aref Ruqaa', serif; }
        ...
        .node-flower {
            ...
            background-position: center;
            z-index: 10;
            transform: scale(0); /* animated via JS */
        ...
        .gallery-cover-flower {
            position: absolute; inset: -20%; /* larger to overlap */
            background-image: url('https://raw.githubusercontent.com/youcef/assets/main/rose_top.png');
            background-size: cover;
            background-position: center;
            z-index: 5;
...
<body x-data="roseData()" x-init="initExperience()">
    ...
    <div id="prelude">
        <h1 class="font-script text-4xl md:text-5xl text-[#B76E79] opacity-0 text-center leading-relaxed" id="prelude-text">
            Every Love Story<br>Has Its Own Garden
        </h1>
    </div>
    ...
    <div id="main-content" style="opacity: 0;">
        <section class="h-[100vh] relative flex flex-col justify-center items-center pb-20">
            <div id="intro-names" class="text-center">
                <h2 class="font-ink text-7xl md:text-9xl text-liquid-gold gs-float" x-text="data.groom_name"></h2>
                <div class="text-3xl text-[#DCAE96] my-6 font-ink gs-float" style="animation-delay: 1s;">&</div>
                <h2 class="font-ink text-7xl md:text-9xl text-liquid-gold gs-float" style="animation-delay: 2s;" x-text="data.bride_name"></h2>
            </div>
        </section>
        ...
        <template x-if="data.story_content">
            <section class="py-[15vh] px-4 min-h-screen relative" id="garden-section">
                <div class="text-center mb-24 gs-fade-up">
                    <h2 class="font-ink text-5xl text-rosegold">حديقة ذكرياتنا</h2>
                </div>
                ...
                <div class="timeline-wrapper max-w-4xl mx-auto" id="timeline-container">
                    ...
                    <template x-for="(line, idx) in splitLines(data.story_content)" :key="idx">
                        <div class="timeline-node gs-story-node">
                            ...
                            <div class="w-1/2" :class="idx % 2 === 0 ? 'pr-12 md:pr-24 text-right ml-auto' : 'pl-12 md:pl-24 text-left mr-auto'">
                                <p class="font-ink text-3xl md:text-4xl leading-relaxed text-liquid-gold" x-text="line"></p>
                            </div>
                        </div>
                    </template>
                </div>
        ...
        <template x-if="data.gallery && data.gallery.length > 0">
            <section class="py-[20vh] px-4 relative z-20" id="gallery-section">
                <div class="text-center mb-16 gs-fade-up">
                    <h2 class="font-ink text-5xl text-rosegold">لحظات مزهرة</h2>
                </div>
        ...
        <section class="py-[20vh] px-4 relative z-20 text-center overflow-hidden" id="date-section">
            <div class="gs-fade-up max-w-4xl mx-auto relative">
                <!-- Simulating petals converging into text -->
                <p class="font-script text-3xl text-[#DCAE96] mb-6">Our Forever Starts On</p>
                <div class="font-ink text-7xl md:text-[8rem] text-petal-texture leading-none drop-shadow-2xl date-text">
                    ...
                    <span class="text-5xl md:text-7xl" x-text="extractMonth(data.event_date)">June</span><br>
        ...
        <template x-if="data.venue_name">
            <section class="py-[20vh] px-4 relative z-20" id="venue-section">
                <div class="text-center mb-16 gs-fade-up">
                    <h3 class="font-ink text-4xl text-rosegold">المكان الذي سيزهر فيه حبنا</h3>
                </div>
                ...
                <div class="gate-wrapper" id="gate-wrapper">
                    ...
                    <div class="absolute inset-0 bg-black/20 flex flex-col items-center justify-center text-center z-10 p-8">
                        ...
                        <template x-if="data.venue_location">
                            <a :href="data.venue_location" target="_blank" class="px-8 py-3 bg-white/30 backdrop-blur-md border border-white/50 text-white rounded-full hover:bg-white hover:text-[#B76E79] transition font-tajawal shadow-xl">
                                استعراض الموقع
                            </a>
                        </template>
                    </div>
        ...
        <section class="py-[20vh] px-4 relative z-20 flex justify-center" id="rsvp-section">
            <div class="w-full max-w-xl text-center relative z-10 gs-fade-up">
                ...
                @if(isset($guest) && $guest)
                    <p class="font-ink text-4xl text-[#B76E79] mb-4">عزيزتنا {{ $guest->name ?? '' }}</p>
                    <h2 class="font-ink text-3xl md:text-4xl mb-12 text-[#4A3C31] leading-tight">وجودك سيضيف وردة أخرى<br>إلى حديقتنا الجميلة</h2>
                @else
                    <h2 class="font-ink text-4xl md:text-5xl mb-12 text-[#B76E79]">تأكيد الحضور</h2>
                @endif

                <template x-if="data.host_message">
                    <p class="font-script text-2xl text-[#DCAE96] mb-8" x-text="data.host_message"></p>
                </template>
                ...
                <div class="glass-card p-8 md:p-14 relative" id="rsvp-card">
                    <div x-show="rsvpStatus === 'success'" class="absolute inset-0 flex flex-col items-center justify-center bg-[var(--pearl-white)]/95 backdrop-blur-md rounded-2xl z-20" style="display: none;">
                        <h3 class="font-ink text-5xl text-rosegold mb-4">تم تأكيد حضورك</h3>
                        <p class="text-[#4A3C31] text-lg">بانتظارك لتشاركينا الفرحة.</p>
                    </div>
        ...
        <footer class="py-[30vh] text-center relative z-20">
            <div class="gs-fade-up">
                <template x-if="data.closing_message">
                    <p class="font-ink text-3xl md:text-4xl text-[#B76E79] mb-16 leading-relaxed" x-text="data.closing_message"></p>
                </template>
                <template x-if="!data.closing_message">
                    <p class="font-ink text-3xl md:text-4xl text-[#B76E79] mb-16 leading-relaxed">بعض اللحظات<br>لا تكتمل إلا بمن نحب</p>
                </template>
                
                <p class="font-ink text-6xl text-liquid-gold">
    ...
    <script>
        // Advanced Intelligent Petal Engine
        class PetalEngine {
            constructor(canvasId) {
                this.canvas = document.getElementById(canvasId);
                this.ctx = this.canvas.getContext('2d');
                this.petals = [];
                this.state = 'prelude'; // prelude -> drift -> target
            ...
            animate() {
                ...
                for (let i = this.petals.length - 1; i >= 0; i--) {
                    ...
                    if (this.state === 'target') {
                        ...
                        // Check if reached center roughly
                ...
                // If target mode finished converging, trigger explosion
        ...
        document.addEventListener('DOMContentLoaded', () => {
            ...
            if(form) {
                form.addEventListener('submit', (e) => {
                    // Find card center to set as target
