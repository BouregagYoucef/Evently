...
<head>
    ...
    <style>
        ...
        .album-page {
            ...
            transform-origin: right center; /* RTL flip origin */
        ...
        .tunnel-wrapper {
            position: relative;
            height: 200vh; /* Scroll length for tunnel */
            overflow: hidden;
        }
        .tunnel-layer {
            position: absolute;
...
<body x-data="blushData()" x-init="initExperience()">
    ...
    <div id="loader">
        ...
        <div id="intro-names" class="opacity-0 flex flex-col items-center">
            ...
            <span class="text-3xl text-[#DCAE96] my-4 font-ink">&</span>
    ...
    <div id="main-content">
        ...
        <section class="h-[100vh] relative flex flex-col justify-end items-center pb-20">
            <div class="gs-fade-up text-center">
                <p class="font-ink text-2xl text-rosegold mb-2">لحظة لا تُنسى</p>
        ...
        <template x-if="data.story_content">
            <section class="py-[20vh] px-4 min-h-screen" id="album-section">
                <div class="text-center mb-16 gs-fade-up">
                    <h2 class="font-ink text-5xl text-rosegold">ذكرياتنا</h2>
                </div>
                ...
                <div class="album-container" id="album-container">
                    <template x-for="(line, idx) in splitLines(data.story_content)" :key="idx">
                        <div class="album-page" :style="`z-index: ${50 - idx};`">
                            <!-- Optional generic images for album if gallery isn't mapped directly -->
                            <div class="w-full h-full border border-[#B76E79]/20 p-8 flex flex-col justify-center items-center">
                                <p class="font-ink text-3xl md:text-4xl leading-relaxed text-[#4A3C31]" x-text="line"></p>
                            </div>
                        </div>
                    </template>
                </div>
        ...
        <section class="tunnel-wrapper bg-[var(--pearl-white)] relative z-20" id="tunnel-section">
            <div class="sticky top-0 h-screen overflow-hidden flex flex-col items-center justify-center">
                <h2 class="font-ink text-5xl md:text-7xl text-rosegold absolute z-50 text-center px-4" id="tunnel-text">
                    رحلة نحو الأبدية
                </h2>
                <!-- Tunnel Layers (Simulated with CSS bordered divs if no floral pngs available, but let's use a public floral png) -->
        ...
        <template x-if="data.bride_message">
            <section class="py-[20vh] px-4 bg-[var(--pearl-white)] relative z-20 flex justify-center">
                <div class="max-w-2xl w-full bg-[var(--soft-ivory)] p-12 md:p-20 shadow-xl border border-[#DCAE96]/30 text-center relative gs-fade-up">
                    ...
                    <p class="font-ink text-3xl md:text-5xl leading-loose text-[#B76E79]" x-text="data.bride_message"></p>
                </div>
            </section>
        </template>
        ...
        <template x-if="data.venue_name">
            <section class="py-[20vh] px-4 bg-[var(--pearl-white)] relative z-20" id="venue-section">
                <div class="text-center mb-12 gs-fade-up">
                    <h3 class="font-ink text-4xl text-rosegold">مكان الفرحة</h3>
                </div>
                ...
                <div class="gate-wrapper" id="gate-wrapper">
                    ...
                    <div class="absolute inset-0 bg-black/30 flex flex-col items-center justify-center text-center z-10 p-8">
                        <h4 class="font-ink text-5xl md:text-7xl text-white mb-6 drop-shadow-lg" x-text="data.venue_name"></h4>
                        <template x-if="data.venue_location">
                            <a :href="data.venue_location" target="_blank" class="px-8 py-3 bg-white/20 backdrop-blur-sm border border-white/50 text-white rounded-full hover:bg-white hover:text-[#B76E79] transition font-tajawal">
                                استعراض الموقع
                            </a>
                        </template>
                    </div>
        ...
        <section class="py-[20vh] px-4 bg-[var(--pearl-white)] relative z-20 flex justify-center" id="rsvp-section">
            <div class="w-full max-w-xl text-center relative z-10 gs-fade-up">
                ...
                @if(isset($guest) && $guest)
                    <p class="font-ink text-3xl text-[#DCAE96] mb-4">عزيزتنا {{ $guest->name ?? '' }}</p>
                    <h2 class="font-ink text-4xl md:text-5xl mb-12 text-rosegold leading-tight">خصصنا لكِ مكاناً في هذا اليوم المميز</h2>
                @else
                    <h2 class="font-ink text-4xl md:text-5xl mb-12 text-rosegold">تأكيد الحضور</h2>
                @endif
                ...
                <div class="glass-pink p-8 md:p-14 relative" id="rsvp-card">
                    <div x-show="rsvpStatus === 'success'" class="absolute inset-0 flex flex-col items-center justify-center bg-[var(--pearl-white)]/90 backdrop-blur-md rounded-3xl z-20" style="display: none;">
                        <h3 class="font-ink text-4xl text-rosegold mb-4">تم تأكيد حضورك بنجاح</h3>
                        <p class="text-[#4A3C31] text-lg">بانتظارك لتشاركينا هذه اللحظات.</p>
                    </div>
        ...
        <footer class="py-[30vh] text-center bg-[var(--pearl-white)] relative z-20">
            <div class="gs-fade-up">
                <template x-if="data.closing_message">
                    <p class="font-ink text-3xl md:text-4xl text-[#B76E79] mb-16" x-text="data.closing_message"></p>
                </template>
                <template x-if="!data.closing_message">
                    <p class="font-ink text-3xl md:text-4xl text-[#B76E79] mb-16">الحب أجمل حين يُشارك<br>ونتمنى أن تكونوا جزءاً من قصتنا</p>
                </template>
                
                <p class="font-ink text-5xl text-rosegold">
    ...
    <script>
        // Custom Canvas Petals Implementation
        class Petal {
            constructor(canvas) {
                this.canvas = canvas;
                    ...
                    if(pages.length > 0) {
                        const albumTl = gsap.timeline({
                            scrollTrigger: {
                                ...
                                start: "top center",
                                end: "+=1500", // extend scroll duration
                                scrub: 1,
                                pin: true
                            }
                        });
                        
                        // Flip each page except the last one
                        for(let i = 0; i < pages.length - 1; i++) {
                            albumTl.to(pages[i], {
                                rotateY: -180, // Flip to the left (RTL)
                                opacity: 0,
                                duration: 1,
                                ease: "power1.inOut"
                            });
