...
<head>
    ...
    <style>
        ...
        .timeline-progress {
            ...
            background: #d4af37; height: 0; /* Will be animated */
        }
...
<body x-data="royalData()" x-init="initExperience()">
    ...
    <div id="loader">
        ...
        <div class="text-center z-10">
            <h1 class="font-royal text-5xl md:text-7xl color-gold mb-6 opacity-0" id="loader-names">
                <span x-text="data.groom_name"></span> <span class="text-3xl">&</span> <span x-text="data.bride_name"></span>
            </h1>
            <p class="text-sm tracking-[0.4em] text-gray-500 uppercase opacity-0" id="loader-text">Loading Experience...</p>
    ...
    <div class="hero-bg" id="hero-bg">
        ...
        <template x-if="!data.hero_video || !data.hero_video.includes('.mp4')">
            <!-- Fallback to image if no valid MP4 is provided -->
    ...
    <div id="main-content">
        <section class="h-[120vh] relative flex justify-center text-center px-4" id="hero-section">
            ...
            <div class="sticky top-[40vh] h-[20vh] w-full max-w-4xl hero-reveal-wrap">
                <h2 class="font-royal text-6xl md:text-8xl text-white mb-6 tracking-wide drop-shadow-2xl">
                    ...
                    <span class="color-gold italic font-light">&</span>
                    <span x-text="data.bride_name"></span>
                </h2>
                <p class="text-sm md:text-lg tracking-[0.3em] uppercase text-gray-300 drop-shadow-md">
        ...
        <template x-if="data.story_text">
            <section class="section-dark py-40 px-4" id="story-section">
                <div class="max-w-3xl mx-auto text-center gs-fade-up">
                    <h3 class="font-royal text-4xl color-gold mb-12 italic">Notre Histoire</h3>
                    <p class="text-xl md:text-2xl leading-relaxed text-gray-300 font-light whitespace-pre-line" x-text="data.story_text"></p>
                </div>
            </section>
        </template>
        ...
        <template x-if="data.gallery && data.gallery.length > 0">
            <section class="section-dark py-32 px-4" id="gallery-section">
                <div class="max-w-7xl mx-auto">
                    <h3 class="text-center font-royal text-4xl color-gold mb-20 italic gs-fade-up">Souvenirs</h3>
        ...
        <template x-if="data.timeline">
            <section class="section-dark py-40 px-4 relative" id="timeline-section">
                <h3 class="text-center font-royal text-4xl color-gold mb-24 italic gs-fade-up">Programme</h3>
                ...
                <div class="max-w-3xl mx-auto relative pb-20">
                    ...
                    <div class="space-y-24">
                        <!-- Parse timeline text into lines -->
                        <template x-for="(line, idx) in splitLines(data.timeline)" :key="idx">
                            <div class="relative flex items-center justify-between w-full gs-timeline-item opacity-0">
                                <!-- Dot -->
                                <div class="absolute left-1/2 -translate-x-1/2 w-4 h-4 rounded-full bg-gold z-10 shadow-[0_0_15px_rgba(212,175,55,0.6)]"></div>
                                
                                <div class="w-5/12" :class="idx % 2 === 0 ? 'text-right pr-8' : 'text-left pl-8 order-last'">
                                    <h4 class="font-royal text-2xl text-white mb-2" x-text="line.split(/[-:]/)[0]"></h4>
                                    <p class="text-gray-400" x-text="line.substring(line.indexOf(line.split(/[-:]/)[0]) + line.split(/[-:]/)[0].length).replace(/^[-:]/, '').trim()"></p>
                                </div>
            ...
            <section class="section-dark py-32 px-4 border-y border-white/5" id="venue-section">
                <div class="max-w-6xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-16 items-center">
                    <div class="gs-fade-up">
                        <h3 class="font-royal text-4xl color-gold mb-6 italic">Le Lieu</h3>
                        <h4 class="text-3xl mb-4 font-light" x-text="data.venue_name"></h4>
                        <p class="text-gray-400 mb-8" x-text="data.venue_address"></p>
                        
                        <p class="text-xl text-white mb-8">{{ \Carbon\Carbon::parse($event->event_datetime)->translatedFormat('d F Y à H:i') }}</p>

                        <template x-if="data.map_link">
                            <a :href="data.map_link" target="_blank" class="inline-block border border-gold text-gold px-8 py-3 hover:bg-gold hover:text-black transition duration-500 uppercase tracking-widest text-sm">
                                Voir la Carte
                            </a>
                        </template>
                    </div>
        ...
        <section class="section-dark py-40 px-4 text-center relative overflow-hidden" id="countdown-section">
            ...
            <div class="max-w-4xl mx-auto relative z-10 gs-fade-up">
                <h3 class="font-royal text-3xl color-gold mb-16 italic">Le grand jour approche</h3>
                <div class="flex justify-center gap-6 md:gap-12">
                    <div class="text-center">
                        <div class="text-5xl md:text-7xl font-light mb-2" x-text="timeLeft.days">00</div>
                        <div class="text-xs tracking-widest uppercase text-gray-500">Jours</div>
                    </div>
                    <div class="text-center">
                        <div class="text-5xl md:text-7xl font-light mb-2" x-text="timeLeft.hours">00</div>
                        <div class="text-xs tracking-widest uppercase text-gray-500">Heures</div>
                    </div>
                    <div class="text-center">
                        <div class="text-5xl md:text-7xl font-light mb-2" x-text="timeLeft.minutes">00</div>
                        <div class="text-xs tracking-widest uppercase text-gray-500">Minutes</div>
                    </div>
                </div>
            </div>
        </section>
        ...
        <section class="section-dark py-40 px-4 bg-[url('https://www.transparenttextures.com/patterns/stardust.png')]" id="rsvp-section">
            <div class="max-w-2xl mx-auto gs-fade-up text-center">
                ...
                @if(isset($guest) && $guest)
                    <h2 class="font-royal text-4xl mb-6 text-white">Bienvenue, {{ $guest->name ?? '' }}</h2>
                    <p class="text-gray-400 mb-12">Nous serions honorés de votre présence à cette occasion spéciale.</p>
                @else
                    <h2 class="font-royal text-4xl mb-6 text-white">Confirmez votre Présence</h2>
                    <p class="text-gray-400 mb-12">Veuillez nous faire savoir si vous serez parmi nous.</p>
                @endif
                ...
                <div class="glass-card p-10 md:p-16 relative" id="rsvp-card">
                    ...
                    <div x-show="rsvpStatus === 'success'" class="absolute inset-0 flex flex-col items-center justify-center bg-black/80 backdrop-blur-md rounded-xl z-20" style="display: none;">
                        <svg class="w-16 h-16 text-green-400 mb-4 animate-bounce" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"></path></svg>
                        <h3 class="font-royal text-2xl text-white">Merci !</h3>
                        <p class="text-gray-400 mt-2">Votre réponse a été enregistrée avec succès.</p>
                    </div>

                    <!-- Native Form from View injected here -->
                    <div id="native-rsvp-wrapper" @submit="handleRSVPSubmit($event)">
        ...
        <footer class="section-dark py-32 text-center border-t border-white/5">
            <template x-if="data.closing_message">
                <p class="font-royal text-2xl color-gold italic mb-12" x-text="data.closing_message"></p>
            </template>
            <p class="text-gray-600 text-xs tracking-[0.3em] uppercase">© {{ $event->title }} Experience</p>
        </footer>
    </div>
    ...
    <div x-show="fullscreenImage" style="display: none;" class="fixed inset-0 z-[10000] bg-black/95 backdrop-blur-sm flex items-center justify-center p-4" @click="fullscreenImage = null">
        <button class="absolute top-8 right-8 text-white text-4xl hover:text-gold transition">&times;</button>
    ...
    <script>
        function royalData() {
            return {
                ...
                handleRSVPSubmit(event) {
                    // We let the form submit normally so the backend processes it.
                    // But we trigger the confetti immediately on click if it's a POST.
                    // Since it causes a page reload, the confetti will show briefly.
                    // If we wanted an SPA feel without reload, we'd use fetch(), but for now we enhance the UX.
                    this.fireConfetti();
                },
        ...
        // Intercept native form submit button click to fire confetti
