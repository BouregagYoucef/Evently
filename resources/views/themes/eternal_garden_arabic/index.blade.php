...
<head>
    ...
    <style>
        ...
        body {
            ...
            background-color: #0d1117; /* Deep night background for later scenes */
            color: #ffffff;
            margin: 0;
            overflow-x: hidden;
...
<body x-data="gardenData()" x-init="initExperience()">
    ...
    <div id="main-content">
        ...
        <section class="h-[120vh] relative flex flex-col justify-center items-center text-center px-4" id="hero-section">
            <div class="sticky top-[40vh] h-[20vh] w-full max-w-4xl">
                <h2 class="font-royal text-6xl md:text-8xl text-white mb-6 drop-shadow-2xl flex flex-col md:flex-row justify-center items-center gap-4">
                    ...
                    <span class="hero-amp opacity-0 text-gold text-5xl md:text-7xl">&</span>
                ...
                <div class="hero-welcome opacity-0 transform translate-y-4 mt-12">
                    <p class="font-royal text-2xl md:text-3xl text-gold mb-2">بكل حب وامتنان</p>
                    <p class="text-lg md:text-xl text-gray-300 drop-shadow-md">ندعوكم لمشاركتنا أجمل أيام حياتنا</p>
                </div>
            </div>
        </section>
        ...
        <template x-if="data.story_cards">
            <section class="section-dark py-40 px-4 overflow-hidden" id="story-section">
                <div class="max-w-5xl mx-auto">
                    <h3 class="text-center font-royal text-4xl md:text-5xl text-gold mb-20 gs-fade-up">القصة</h3>
                    <div class="space-y-12">
                        <!-- We split text by double newline to create cards -->
                        <template x-for="(cardText, idx) in splitCards(data.story_cards)" :key="idx">
                            <div class="story-card opacity-0 transform" :class="idx % 2 === 0 ? 'translate-x-[100px]' : '-translate-x-[100px] ml-auto'" style="max-w: 600px;">
                                ...
                                <p class="text-xl md:text-2xl leading-loose text-gray-200 font-light" x-text="cardText"></p>
                            </div>
                        </template>
                    </div>
                </div>
        ...
        <template x-if="data.gallery && data.gallery.length > 0">
            <section class="section-dark py-32 px-4" id="gallery-section">
                <div class="max-w-7xl mx-auto">
                    <h3 class="text-center font-royal text-4xl md:text-5xl text-gold mb-20 gs-fade-up">معرض الذكريات</h3>
        ...
        <section class="section-dark py-40 px-4 text-center relative overflow-hidden" id="countdown-section">
            ...
            <div class="max-w-4xl mx-auto relative z-10 gs-fade-up">
                <h3 class="font-royal text-4xl md:text-5xl text-white mb-16 drop-shadow-lg">الوقت المتبقي</h3>
                <div class="flex justify-center gap-6 md:gap-12 dir-ltr" style="direction: ltr;">
                    <div class="text-center">
                        <div class="flip-box mb-4">
                            <div class="text-5xl md:text-7xl font-bold font-mono text-white" x-text="timeLeft.days">00</div>
                        </div>
                        <div class="text-lg text-gold font-royal">يوم</div>
                    </div>
                    <div class="text-center">
                        <div class="flip-box mb-4">
                            <div class="text-5xl md:text-7xl font-bold font-mono text-white" x-text="timeLeft.hours">00</div>
                        </div>
                        <div class="text-lg text-gold font-royal">ساعة</div>
                    </div>
                    <div class="text-center">
                        <div class="flip-box mb-4">
                            <div class="text-5xl md:text-7xl font-bold font-mono text-white" x-text="timeLeft.minutes">00</div>
                        </div>
                        <div class="text-lg text-gold font-royal">دقيقة</div>
                    </div>
                </div>
            </div>
        </section>
        ...
        <template x-if="data.venue_name">
            <section class="section-dark py-40 px-4 border-y border-white/5 relative" id="venue-section">
                <!-- Subtle background image of the venue -->
                ...
                <div class="max-w-4xl mx-auto text-center relative z-10 gs-fade-up">
                    <h3 class="font-royal text-3xl text-gray-400 mb-6">موقع الحفل</h3>
                    <!-- Gold Engraved Name -->
                    ...
                    <div class="inline-block border border-gold/30 bg-black/50 backdrop-blur-sm p-6 rounded-lg mb-12">
                        <p class="text-2xl text-white font-royal">{{ \Carbon\Carbon::parse($event->event_datetime)->translatedFormat('l، d F Y') }}</p>
                    ...
                    <div>
                        <template x-if="data.map_link">
                            <a :href="data.map_link" target="_blank" class="inline-block border-2 border-gold text-gold px-12 py-4 hover:bg-gold hover:text-black transition duration-500 text-xl font-royal rounded-full">
                                عرض الموقع
                            </a>
                        </template>
                    </div>
        ...
        <section class="section-dark py-40 px-4 relative" id="rsvp-section">
            ...
            <div class="max-w-2xl mx-auto gs-fade-up text-center relative z-10">
                ...
                @if(isset($guest) && $guest)
                    <h2 class="font-royal text-4xl md:text-5xl mb-6 text-gold">مرحباً {{ $guest->name ?? '' }}</h2>
                    <p class="text-gray-300 text-xl mb-12">وجودك سيجعل فرحتنا أجمل وأكثر اكتمالاً.</p>
                @else
                    <template x-if="data.host_message">
                        <div>
                            <p class="text-gray-300 text-xl mb-12 whitespace-pre-line" x-text="data.host_message"></p>
                        </div>
                    </template>
                    <template x-if="!data.host_message">
                        <h2 class="font-royal text-4xl md:text-5xl mb-12 text-gold">تأكيد الحضور</h2>
                    </template>
                @endif
                ...
                <div class="glass-card p-10 md:p-16 relative" id="rsvp-card">
                    ...
                    <div x-show="rsvpStatus === 'success'" class="absolute inset-0 flex flex-col items-center justify-center bg-[#0d1117]/95 backdrop-blur-xl rounded-2xl z-20" style="display: none;">
                        <svg class="w-24 h-24 text-gold mb-6 animate-pulse" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>
                        <h3 class="font-royal text-4xl text-white gold-engrave">تم تسجيل حضورك</h3>
                        <p class="text-gray-400 mt-4 text-xl">بانتظارك بكل حب في يومنا الموعود.</p>
                    </div>
        ...
        <footer class="section-dark py-40 text-center border-t border-white/5 relative overflow-hidden">
            <!-- Large faded names in background -->
            ...
            <div class="relative z-10">
                <template x-if="data.closing_message">
                    <p class="font-royal text-3xl md:text-4xl text-gray-300 mb-12 leading-loose" x-text="data.closing_message"></p>
                </template>
    ...
    <div x-show="fullscreenImage" style="display: none;" class="fixed inset-0 z-[10000] bg-black/95 backdrop-blur-md flex items-center justify-center p-4" @click="fullscreenImage = null">
        <button class="absolute top-8 left-8 text-white text-5xl hover:text-gold transition">&times;</button>
        ...
        function gardenData() {
            return {
                ...
                startExperience() {
                    ...
                    // Cinematic Hero Reveal Sequence
                    const tl = gsap.timeline();
                ...
                // Custom Petals Engine (HTML5 Canvas)
                initPetals() {
